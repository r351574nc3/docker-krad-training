...
http://mirror.symnds.com/software/Apache/tomcat/tomcat-7/v7.0.54/bin/apache-tomcat-7.0.54.tar.gz
...
# Verify Download
RUN echo "e43771bc8c6a10956495d13300567244  /tmp/apache-tomcat-$TOMCAT_VERSION.tar.gz" | \
    md5sum -c

RUN tar -xzf /tmp/apache-tomcat-$TOMCAT_VERSION.tar.gz
RUN mv /tmp/apache-tomcat-$TOMCAT_VERSION /usr/share
RUN ln -s /usr/share/apache-tomcat-$TOMCAT_VERSION /usr/share/tomcat
